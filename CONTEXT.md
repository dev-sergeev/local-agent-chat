...
**Sandbox**:
Принадлежащее одному Chat локальное рабочее пространство агента, в котором агент работает с файлами и выполняет Python и shell-команды. Оно сохраняет файлы между Turn, но не является security sandbox: изоляцию должен обеспечить окружающий контейнер или VM. Ключи приложения не передаются в environment команд.
