...
OpenAI SDK retries and the native GigaChat adapter cover transient HTTP requests. GigaChat uses a ready access token and a configurable base URL; its SDK retry is disabled because it can replay started streams. The adapter retries asynchronous inference only before the first emitted chunk and applies request and chunk deadlines. Agent, summary and title models share the same provider factory. Zero-chunk stream retry wraps only the model handler or summary inference; it never restarts tools, a graph or the whole Turn. Summarization bypasses the upstream nested retry runnable. The main model's call limit ends a runaway tool loop with an error and runtime rollback.
