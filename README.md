...
Можно также установить собранный wheel: `pipx install --python python3.12 ./dist/local_agent_chat-0.2.2-py3-none-any.whl`. Альтернатива pipx — `python3.12 -m venv .venv`, активация окружения и `python -m pip install local-agent-chat`.
...
Если файловая система возвращает `No locks available` или не поддерживает `flock`, запуск использует атомарное создание каталога блокировки. Второй экземпляр с теми же данными по-прежнему не запускается. После принудительного завершения процесса может остаться `APP_DATA_DIR/.localchat.lock.d`: удаляйте этот пустой каталог только после остановки всех экземпляров, использующих эти данные. При обычном завершении он удаляется автоматически.

Если постоянный каталог находится на NFS без рабочих блокировок SQLite, добавьте в `.env`:
...
Это явный режим для одного экземпляра `localchat run`. Без настройки или при значении `0` используются обычные блокировки SQLite; другие значения отклоняются. После изменения настройки нужен полный перезапуск. Относительный `APP_DATA_DIR` остаётся привязан к каталогу конфигурации: убедитесь, что он расположен на постоянном NFS, а не на временном диске пода.

В этом режиме все хранилища отключают файловые блокировки SQLite и последовательно выполняют чтения и транзакции для каждого файла БД. Память и профили используют общую защиту `checkpoints.sqlite3`; сессии Chainlit защищены целиком, включая откат и закрытие. Повторный Stop дожидается завершения уже выполняющегося SQL, прежде чем освободить БД. Используются `journal_mode=DELETE` и `synchronous=EXTRA`. Существующая WAL-база отклоняется без автоматического преобразования: остановите все экземпляры, перенесите согласованный комплект БД и журналов на диск с рабочими блокировками, выполните преобразование в DELETE и только затем возвращайте данные на NFS. Не удаляйте файлы `-wal`, `-shm` и `-journal` вручную.

Не открывайте эти БД параллельно другим процессом, SQLite-консолью или приложением. После аварии сначала подтвердите остановку прежнего родительского процесса, дочернего сервера и других подов с теми же данными; только затем можно удалить пустой `.localchat.lock.d` командой `rmdir`. Отсутствие PID в текущем контейнере или возраст каталога не доказывают, что владелец остановлен. Автоматического удаления stale-lock нет.

Режим сохраняет данные непосредственно на NFS; надёжность `fsync` и восстановления при сбоях сервера зависит от NFS. Он не создаёт общей атомарной транзакции между тремя БД и файлами приложения. Сохранение размеров файлов и строка `Press Ctrl+C to stop.` не заменяют проверку восстановления: после испытания на отдельном каталоге проверьте готовность HTTP, ожидаемые записи и `PRAGMA integrity_check` всех трёх БД при остановленном приложении. Подробности ограничений: [SQLite URI](https://www.sqlite.org/uri.html#uriqueryparameters), [SQLite на сетевых файловых системах](https://www.sqlite.org/useovernet.html).

Из исходного дерева можно выполнить проверку на конкретном NFS (нужны зависимости `.[test]`):

```bash
LOCALCHAT_TEST_NFS_DIR=/путь/к/существующему/каталогу/на/nfs \
  python -m pytest -q tests/test_sqlite_nfs.py -k real_nfs
```

Тест создаёт собственный временный каталог, проверяет все три БД, параллельные операции, повторное открытие и восстановление после `kill -9` тестового процесса; пользовательские БД не открывает. Пересоздание пода и сбои самого NFS нужно проверять отдельно в целевой среде.
...
Токены обоих профилей задаются в `.env`. Модели используются для ответа, инструментов и суммаризации. За одно сообщение агенту предписано вызывать не более одного инструмента и ждать результата перед следующим вызовом.

По умолчанию название чата берётся из начала первого запроса без дополнительного обращения к модели. Для генерации короткого названия моделью добавьте `APP_GENERATE_CHAT_TITLES=true` в `.env` и перезапустите приложение. Любое другое значение или отсутствие переменной отключает генерацию. Уже сохранённые названия остаются прежними.
